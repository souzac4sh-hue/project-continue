...
function getUrgencyText(product: Product): string | null {
  // Generate a stable pseudo-random number from product id
...
export function ProductCard({ product }: { product: Product }) {
  ...
  // Stable review count from product id
  const seed = product.id.split('').reduce((acc, c) => acc + c.charCodeAt(0), 0);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="p-3.5">
          <h3 className="font-serif font-semibold text-foreground group-hover:text-primary transition-colors text-sm leading-tight line-clamp-2">
            {product.name}
          </h3>
          ...
          {urgency && (
            <div className="mt-1.5 flex items-center gap-1">
              <span className="text-[10px] font-semibold text-primary/90 leading-tight">{urgency}</span>
            </div>
          )}
            ...
            <div>
              {hasPromo ? (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-primary font-bold text-sm">R$ {product.promotionPrice!.toFixed(2)}</span>
                </div>
              ) : (
                <span className="text-primary font-bold text-sm">R$ {product.price.toFixed(2)}</span>
              )}
              <p className="text-[10px] text-muted-foreground mt-0.5 flex items-center gap-1">
                <svg viewBox="0 0 512 512" className="h-2.5 w-2.5 fill-primary/60" aria-hidden="true"><path d="M242.4 292.5C247.8 287.1 257.1 287.1 262.5 292.5L339.5 369.5C347.6 377.6 342.7 391.2 331.2 392.4C252.7 400.7 183.6 358.3 152.5 295.4L134.6 313.3C129.2 318.7 119.9 318.7 114.5 313.3L37.5 236.3C32.1 230.9 32.1 221.6 37.5 216.2L114.5 139.2C119.9 133.8 129.2 133.8 134.6 139.2L211.6 216.2C217 221.6 217 230.9 211.6 236.3L193.7 254.2C## 217.1 267.9 242.4 278.1 269.5 283.5L242.4 292.5zM464 256A208 208 0 1 1 48 256a208 208 0 1 1 416 0zM0 256a256 256 0 1 0 512 0A256 256 0 1 0 0 256z"/></svg>
                À vista no Pix
              </p>
            </div>
            <span className="flex items-center gap-1 text-[10px] font-bold gold-gradient text-primary-foreground px-2.5 py-1.5 rounded-lg group-hover:shadow-md group-hover:shadow-primary/20 transition-all">
